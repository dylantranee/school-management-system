...
import { Users, LogOut, Calendar, Wallet, BookOpen, CheckSquare } from 'lucide-react';
...
export const DashboardLayout = () => {
  ...
  const getAccountName = () => {
    ...
    if (user.role === 'Student' && user.profile) {
      return `${user.profile.student_first_name} ${user.profile.student_last_name}`;
    }
    if (user.role === 'Staff' && user.profile) {
      return `${user.profile.staff_first_name} ${user.profile.staff_last_name}`;
    }
    if (user.role === 'Admin') {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      {/* Sidebar */}
      <aside className="w-[250px] bg-secondary border-r border-border flex flex-col flex-shrink-0">
        ...
        <div className="p-4 border-t border-border">
          <div className="flex items-center justify-between bg-background p-3 rounded-md border border-border shadow-sm">
            <div className="flex flex-col overflow-hidden">
              <span className="text-[13px] font-semibold text-foreground truncate" title={getAccountName()}>
                {getAccountName()}
              </span>
      ...
      {/* Main Content Area */}
      <main 
        className="flex-1 relative overflow-y-auto bg-background"
        style={{
